...
(async()=>{
  ...
  try {
    ...
    for(const id of Object.keys(world.maps)) {
      ...
      await page.evaluate(()=>{__qa.state.nextWaveReadyAt=__qa.state.time+.01;__qa.state.paused=false;__qa.update(.02);__qa.state.paused=true;});
      assert(await page.evaluate(()=>__qa.state.waveActive),'wave starts automatically after the adjustment countdown');
    ...
    await page.locator('#stash-loot').click();
    await page.locator('#element-inventory button').first().click();
    assert(await page.locator('#core-action-bar').isVisible(),'selected warehouse core exposes contextual actions');
    await page.mouse.move(420,55);await page.waitForTimeout(220);
    await page.screenshot({path:path.join(root,'artifacts','core-sale-844.png')});
    await page.locator('#sell-core-button').click();
    assert.equal(await page.locator('#element-inventory button').count(),0,'sold core leaves the warehouse');
    assert.equal(await page.evaluate(()=>__qa.state.gold),goldBeforeSale+30,'selling a core grants spirit currency');
